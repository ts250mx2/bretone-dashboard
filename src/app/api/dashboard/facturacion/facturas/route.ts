...
export async function POST(request:NextRequest){
  ...
  try{
    ...
    try{
      ...
      const [header]=await connection.execute(`INSERT INTO tblFacturasDashboard (Tipo,FechaOperacion,RFC,RazonSocial,CodigoPostal,RegimenFiscal,UsoCFDI,Correo,Periodicidad,Meses,Anio,NumTickets,BaseGravable,IVA,IEPS,ConsumoFacturable,Propinas) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,[mode,operationDate,recipient.rfc,recipient.razonSocial,recipient.codigoPostal,recipient.regimenFiscal,recipient.usoCFDI,recipient.correo||null,periodic?'01':null,periodic?operationDate.slice(5,7):null,periodic?Number(operationDate.slice(0,4)):null,details.length,totals.base,totals.iva,totals.ieps,totals.consumption,totals.tips]);
