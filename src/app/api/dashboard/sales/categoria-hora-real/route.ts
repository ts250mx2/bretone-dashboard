...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Execute the SQL query with range placeholders using HOUR(Fecha) for real time
    const sqlQuery = `
      SELECT 
      ...
      WHERE Cancelada = 0 
        AND ${dateFilter}
      GROUP BY Categoria, HOUR(Fecha), tblCategorias.IdCategoria 
      ORDER BY HOUR(Fecha), tblCategorias.IdCategoria
    `;
    ...
    console.error('Error in api/dashboard/sales/categoria-hora-real:', error);
