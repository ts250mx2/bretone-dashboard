...
import styles from './categoria-hora.module.css';
...
export default function SalesByCategoryHourReport() {
  ...
  const fetchReport = useCallback(async (from: string, to: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/dashboard/sales/categoria-hora?${params}`);
  ...
  const handleMouseMove = (h: number, e: React.MouseEvent<SVGElement>) => {
    // Measure coordinates relative to .chartCard (parent of parent of SVG)
    ...
    if (rect) {
      setTooltip({
        ...
        y: e.clientY - rect.top - 15, // 15px offset above mouse
      });
    }
  };
  ...
  const handleExportExcel = () => {
    ...
    const rows = hoursList
      .map(h => {
        ...
        // Build excel row columns dynamically
      ...
      // Only keep hours with sales or show all
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Ventas Categoria-Hora');
    XLSX.writeFile(workbook, `Ventas_Categoria_Hora_${dateFrom}_a_${dateTo}.xlsx`);
  };
    ...
    <div className={styles.container}>
      ...
      <header className={styles.header}>
        <div className={styles.titleGroup}>
          ...
          <div>
            <h1>Ventas por Categoría y Hora</h1>
            <p className={styles.subtitle}>Análisis del facturado acumulado por grupo de menú y bloque horario</p>
          </div>
        </div>
      ...
      <div className={styles.chartCard} style={{ position: 'relative' }}>
        <div className={styles.chartHeader}>
          <div>
            <h3>Distribución Horaria por Categoría</h3>
            <p>Representación visual de ventas acumuladas por grupo comercial y hora</p>
          </div>
        </div>
        ...
        <div className={styles.chartWrapper}>
          {loading ? (
            <div className={styles.chartLoading}>
              ...
              <span>Analizando distribución horaria...</span>
            </div>
          ) : data.length === 0 ? (
        ...
        {/* Floating Tooltip Component - placed outside chartWrapper to avoid clipping */}
        {tooltip && (
          <div
      ...
      <div className={styles.tableCard}>
        <div className={styles.tableTitleBlock}>
          <h3>Detalle Comercial de Ventas</h3>
          <p>Tabla dinámica cruzando categorías y horas de facturación</p>
        </div>
              ...
              <tbody>
                {hoursList
                  .map(h => {
                    ...
                    // Skip displaying completely empty rows in the table to keep it compact
